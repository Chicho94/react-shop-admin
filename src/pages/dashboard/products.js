...
const products = () => {
  ...
  let products = useFetch(endPoints.products.getProducts(PRODUCT_LIMIT, offset));
        ...
        <div className="-my-2 overflow-x-auto sm:-mx-6 lg:-mx-8">
          <div className="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
            <div className="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {products?.map((product) => (
                    <tr key={`Product-item-${product.id}`}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-900">{product.category.nam}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
              ...
              <Pagination quantity={PRODUCT_LIMIT} setOffset={setOffset} offset={offset} />
            </div>
          </div>
        </div>
      </div>
